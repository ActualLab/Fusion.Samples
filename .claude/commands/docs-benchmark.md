...
|------------|--------------------------------------|
| `docs/Performance.md` | The master mirror. Header **version + date**; PerformanceTest tables (multi + single, SQLite/Postgres) + speedups; Benchmark.cmd Local/Remote tables; RpcBenchmark Calls / Streams / Throughput (recompute GB/s & MB/s) + the two `<BarChart>` `data:[...]` arrays; the Micro-Benchmarks + Interception tables; Docker Calls/Streams + their two `<BarChart>` arrays; Redis. **Latency tables (native + Docker) intentionally live only in Benchmarks.md — do not mirror them here.** Keep the Benchmarks.md back-link at the tail. |
