...
public class TodoApi(IAuth auth, ITodoBackend backend, ICommander commander) : ITodoApi
{
    ...
    public virtual async Task<TodoItem> AddOrUpdate(Todos_AddOrUpdate command, CancellationToken cancellationToken = default)
    {
        ...
        var scope = await GetScope(session, cancellationToken).ConfigureAwait(false);
        return await commander.Call(new TodoBackend_AddOrUpdate(scope, todo), cancellationToken).ConfigureAwait(false);
    }
    ...
    public virtual async Task Remove(Todos_Remove command, CancellationToken cancellationToken = default)
    {
        ...
        var scope = await GetScope(session, cancellationToken).ConfigureAwait(false);
        await commander.Call(new TodoBackend_Remove(scope, id), cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        var scope = await GetScope(session, cancellationToken).ConfigureAwait(false);
        var item = await backend.Get(scope, id, cancellationToken).ConfigureAwait(false);
    ...
    {
        var scope = await GetScope(session, cancellationToken).ConfigureAwait(false);
        return await backend.ListIds(scope, count, cancellationToken).ConfigureAwait(false);
    }

    public virtual async Task<TodoSummary> GetSummary(Session session, CancellationToken cancellationToken = default)
    {
        var scope = await GetScope(session, cancellationToken).ConfigureAwait(false);
        return await backend.GetSummary(scope, cancellationToken).ConfigureAwait(false);
    }

    // Private methods

    private async ValueTask<string> GetScope(Session session, CancellationToken cancellationToken)
    {
